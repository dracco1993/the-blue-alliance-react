...
export default class MatchBreakdown2019 extends MatchBreakdown {
  ...
  getRocketShipView(counts) {
    const { panelCount, cargoCount } = counts;
  ...
  getRocketCompletedImage(breakdown) {
    const nearRocket = this.getRocketShipDataFor(breakdown, "Near");
    const farRocket = this.getRocketShipDataFor(breakdown, "Far");

    const allPanels = nearRocket.panelCount + farRocket.panelCount == 12;
    const allCargo = nearRocket.cargoCount + farRocket.cargoCount == 12;

    const doubleRocket = allPanels && allCargo;

    if (doubleRocket) {
      return this.doubleCheckImage();
    } else {
